    ...
    <div className={'ml-10 flex items-baseline space-x-4'}>
      {divisions.all.map((division) => (
        <Menu as={'div'} className={'relative inline-block text-left'} key={division.nhlId}>
          ...
          >
            <Menu.Items className={'origin-top-right absolute right-0 mt-2 w-64 rounded-md shadow-lg bg-white ring-1 ring-gray-300 divide-y divide-gray-200 focus:outline-none'}>
              {getTeamsForDivision(division).map((team) => (
                <Menu.Item key={team.getId()}>
                  {({ active }) => (
                    <Link href={`/teams/${team.getSlug()}`}>
                      <a className={`${active ? 'bg-gray-200' : ''} hover:bg-gray-200 text-gray-900 group flex items-center px-4 py-2`}>
